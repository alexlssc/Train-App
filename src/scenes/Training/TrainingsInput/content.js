...
const TrainingsInputContent = () => {
  ...
  const handleAddAttendee = e => {
    ...
    if (!checkIfUserExists(playerKey)) {
      db.collection('trainings')
        ...
        .catch(() => {
          dispatch(
            snackbarOn('Erreur: Joueur non ajouté', 'error', new Date()),
          );
        });
    } else {
  ...
  const handleUpdatePlayerAttendee = async (key, value) => {
    ...
    const newOverallPerformance = await updateOverallPerformance().then(
      result => {
        return result;
      },
    );
  ...
  const updateOverallPerformance = async () => {
    let output;
    let trainingDoc = await db
      .collection('trainings')
      .doc(id)
      .get();
    let allPerformances = Object.keys(trainingDoc.data().playerAttendees).map(
      playerValues =>
  ...
  const handleDeletePlayer = async key => {
    ...
    await Promise.all(promises)
      ...
      .catch(() => {
        dispatch(
          snackbarOn('Erreur: Joueur non supprimé', 'error', new Date()),
        );
      });
  ...
  const handleUpdateDate = () => {
    try {
      if (hasNumber(trainingData.training[id].date)) {
        ...
        db.collection('trainings')
          ...
          .catch(() => {
            dispatch(
              snackbarOn('Erreur: Date non mis à jour', 'error', new Date()),
            );
          });
      } else {
  ...
  const pushChangeName = () => {
    try {
      db.collection('trainings')
        .doc(id)
        .set(
          {
            name: trainingData.training[id].name,
          },
          { merge: true },
        )
        .then(() => {
          dispatch(snackbarOn('Titre mis à jour', 'success', new Date()));
        })
        .catch(() => {
          dispatch(
            snackbarOn('Erreur: Titre non mis à jour', 'error', new Date()),
          );
        });
    } catch (e) {
      dispatch(snackbarOn('Erreur avec titre', 'error', new Date()));
    }
  };
    ...
    <React.Fragment>
      ...
      <DatePicker
        datePickerPropsValue={
          trainingData.training[id] != null
            ? trainingData.training[id].date
            : null
        }
      ...
      />
      <ChangeName
          name={trainingData.training[id] != null ? trainingData.training[id].name : null}
          handleChangeName={handleChangeName}
          pushChangeName={pushChangeName}
      />
      <Paper>
        <FormControl variant="outlined" className={classes.form}>
          ...
          >
            ...
            {Object.entries(listPlayers.player).map(([key, playerObject]) => (
              <MenuItem key={key} value={[playerObject, key]}>
                {playerObject['lastName'] + ' ' + playerObject['firstName']}
              </MenuItem>
            ))}
          </Select>
        </FormControl>
      ...
      <TableTraining
        playerAttendees={
          trainingData.training[id] != null
            ? trainingData.training[id].playerAttendees
            : 'Waiting'
        }
